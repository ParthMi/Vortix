...
// Dynamically generate metadata based on blog post
export async function generateMetadata({ params }: BlogDetailsPageProps): Promise<Metadata> {
  ...
  if (!blog) {
    return {
      title: "Blog Not Found | Vortix Enterprise",
      description: "The blog you're looking for does not exist.",
    };
  }

  return {
    title: `${blog.title} | Vortix Enterprise`,
    description: blog.summary || "Blog post",
...
const BlogDetailsPage = ({ params }: any) => {
  const blogId = parseInt(params.id, 10);
  const blog: any | undefined = blogData.find((b) => b.id === blogId);
